...
Mat  place_center(Mat number, Size new_size)
{
	int big = max(number.cols, number.rows);
	int  dx = (big - number.cols) / 2;
	int  dy = (big - number.rows) / 2;
	Mat  square(big, big, number.type(), Scalar(255));	// 정방영상

	Rect center_rect(Point(dx, dy), number.size());	// 중심영역 사각형
	Mat  center_square = square(center_rect);		// 중심 참조 영상
	number.copyTo(center_square);
...
int main() {
	...
	string image_file = "../image/train_data1.png";
	...
	for (int i = 0, k = 0; i<Nclass; i++) {
		for (int j = 0; j< Nsample; j++, k++)
		{
			...
			Mat  data = place_center(num, size);	// 셀 중심에 숫자 배치 
	...
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 6; j++)
		{
			...
			Mat  data = place_center(num, size);
